...
if (strlen($_SESSION['bpmsaid']==0)) {
  header('location:logout.php');
    ...
    if($_GET['delid']){
        ...
        echo "<script>window.location.href='accepted-booking.php'</script>";
    }
    ...
    if($_GET['paidid']){
		...
		mysqli_query($con,"UPDATE tblbook tb SET tb.tblbook_status = 'Paid' WHERE tb.tblbook_bookingnumber ='$pid'");
        ...
        try {
            ...
            $mail->Body    = '<p style="padding: 10px 20px; border-left: 10px groove green; color: white; background: #000; font-size: 18px">Hello <span style="color: crimson"><b>'.$userid_name.'</b>!</span><br><br>This is to inform you that your booking is paid!, <br>['.$pid.'] </p>';
...
<html>
<head>
<title>BeautyCiti Salon || Today List</title>
...
<body class="cbp-spmenu-push">
	<div class="main-content">
		...
		<div id="page-wrapper">
			<div class="main-page">
				<div class="tables">
					<h3 class="title1">Today List</h3>
					
					
				
					<div class="table-responsive bs-example widget-shadow">
						<div class="row" style="background: #f2f2f2">
							<div class="col-md-6"><br><h4>Today List:</h4></div>
							...
							if(isset($_GET['from'])){
								if(isset($_GET['to'])){
									...
									if($user['type'] == 0){
										...
										WHERE tb.tblbook_status = 'Accepted' AND tb.tblbook_bookingdate BETWEEN '$from' AND '$to'
									...
									if($user['type'] == 1){
										...
										$ret=mysqli_query($con,"SELECT * FROM tblbook tb
										INNER JOIN tbl_users tu ON tu.users_id=tb.tblbook_userid 
										WHERE tb.tblbook_status = 'Accepted' AND tb.tblbook_staffid = '$staff_id' AND tb.tblbook_bookingdate BETWEEN '$from' AND '$to'
										GROUP BY tb.tblbook_bookingnumber
										ORDER BY tb.tblbook_id DESC");
								...
								if($user['type'] == 0){
									...
									$ret=mysqli_query($con,"SELECT * FROM tblbook tb
									INNER JOIN tbl_users tu ON tu.users_id=tb.tblbook_userid 
									WHERE tb.tblbook_status = 'Accepted' AND DATE(tb.tblbook_bookingdate) = '$today_date'
									GROUP BY tb.tblbook_bookingnumber
									ORDER BY tb.tblbook_id DESC");
								...
								if($user['type'] == 1){
									...
									$ret=mysqli_query($con,"SELECT * FROM tblbook tb
									INNER JOIN tbl_users tu ON tu.users_id=tb.tblbook_userid 
									WHERE tb.tblbook_status = 'Accepted' AND tb.tblbook_staffid = '$staff_id' AND DATE(tb.tblbook_bookingdate) = '$today_date'
									GROUP BY tb.tblbook_bookingnumber
									ORDER BY tb.tblbook_id DESC");
							...
							<?php if($row['tblbook_status']==""){ ?>
                     		...
                     		<?php } else { ?>
                                ...
                                <td>
									...
									<a href="accepted-booking.php?paidid=<?php echo $row['tblbook_bookingnumber'];?>&uid=<?php echo $row['tblbook_userid'];?>" class="btn btn-success">Paid</a>
			...
			$(document).ready(function(){
				$('#from').on('change', function(){
					...
					$('#to').on('change', function(){
						...
						window.location.href = "accepted-booking.php?from="+from+"&to="+to;
					})
				})
			})
		</script>
