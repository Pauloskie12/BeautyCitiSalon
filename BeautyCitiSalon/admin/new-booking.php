...
if (strlen($_SESSION['bpmsaid']==0)) {
  header('location:logout.php');
	...
	if($_GET['delid']){
		...
		$message = "[".$sid."] Your booking is rejected!";
...
<html>
<head>
<title>BeautyCiti Salon || New Book</title>
...
<body class="cbp-spmenu-push">
	<div class="main-content">
		...
		<div id="page-wrapper">
			<div class="main-page">
				<div class="tables">
					<h3 class="title1">New Book</h3>
					
					
				
					<div class="table-responsive bs-example widget-shadow">
						<div class="row" style="background: #f2f2f2">
							<div class="col-md-6"><br><h4>New Book:</h4></div>
							<div class="col-md-6">
								<div class="row">
						...
						<table class="table table-bordered" id="example1"> <thead> 
							<tr> 
								<th>#</th> 
							...
							if(isset($_GET['from'])){
								if(isset($_GET['to'])){
									...
									if($user['type'] == 0){
										...
										WHERE tb.tblbook_status = '' AND tb.tblbook_bookingdate BETWEEN '$from' AND '$to'
									...
									if($user['type'] == 1){
										...
										$ret=mysqli_query($con,"SELECT * FROM tblbook tb
										INNER JOIN tbl_users tu ON tu.users_id=tb.tblbook_userid 
										WHERE tb.tblbook_status = '' AND tb.tblbook_staffid = '$staff_id' AND tb.tblbook_bookingdate BETWEEN '$from' AND '$to'
										GROUP BY tb.tblbook_bookingnumber
										ORDER BY tb.tblbook_id DESC");
								...
								if($user['type'] == 1){
									...
									$ret=mysqli_query($con,"SELECT * FROM tblbook tb
									INNER JOIN tbl_users tu ON tu.users_id=tb.tblbook_userid 
									WHERE tb.tblbook_status = '' AND tb.tblbook_staffid = '$staff_id'
									GROUP BY tb.tblbook_bookingnumber
									ORDER BY tb.tblbook_id DESC");
						 	...
						 	<tr> 
								...
								<?php if($row['tblbook_status']==""){ ?>
                     			...
                     			<?php } else { ?>
                      				...
                      				<td><a href="view-booking.php?viewid=<?php echo $row['tblbook_bookingnumber'];?>&userid=<?=$row['users_id'];?>" class="btn btn-primary">View</a>
										<a href="new-booking.php?acpid=<?php echo $row['tblbook_bookingnumber'];?>&userid=<?=$row['users_id'];?>" class="btn btn-success" onClick="return confirm('Are you sure you want to accept?')">Accept</a>
										<a href="new-booking.php?delid=<?php echo $row['tblbook_bookingnumber'];?>&userid=<?=$row['users_id'];?>" class="btn btn-danger" onClick="return confirm('Are you sure you want to reject?')">Reject</a>
									</td> 
									</tr>   
								<?php 
								$cnt=$cnt+1;
			...
			$(document).ready(function(){
				$('#from').on('change', function(){
					...
					$('#to').on('change', function(){
						...
						window.location.href = "new-booking.php?from="+from+"&to="+to;
					})
				})
			})
		</script>
